...
Deployment automation lives in `.github/workflows/`.
...
- GitHub Actions deploys by SSH-ing into the VPS, syncing `main` into the checkout directory, and running `docker compose up -d --build --remove-orphans`.
...
Never store secrets outside controlled defaults. Override `JWT_SECRET_KEY`, admin credentials, and database settings through environment variables. Keep `.env` only on the VPS in the repo root used for deployment, and treat `/var/log/emr/app.log` as operational data rather than source-controlled output. GitHub Actions deploy requires `VPS_HOST`, `VPS_USER`, `VPS_SSH_KEY`, and `VPS_DEPLOY_PATH`; missing secrets fail the job before SSH setup.
